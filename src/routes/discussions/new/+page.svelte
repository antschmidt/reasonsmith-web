<script lang="ts">
	...
	// Tags are now generated by AI from good faith analysis
	let aiGeneratedTags = $state<string[]>([]);
	...
	async function loadExistingDiscussion(id: string) {
		try {
			...
			if (discussion && discussion.draft_version?.length > 0) {
				...
				// Load AI-generated tags from good faith analysis if available
				if (draftVersion.tags) {
					aiGeneratedTags = draftVersion.tags;
				}
				...
				if (draftVersion.good_faith_score !== null) {
					...
					// Extract tags from good faith analysis if present
					if (draftVersion.good_faith_analysis?.tags) {
						aiGeneratedTags = draftVersion.good_faith_analysis.tags;
					}
				}
		...
		try {
			...
			const { data: discData, error: discError } = await nhost.graphql.request(
				CREATE_DISCUSSION_WITH_VERSION,
				{
					title: discTitle,
					description: content || '',
					tags: validateTags(aiGeneratedTags),
					sections: [],
					claims: [],
	...
	// Tags are now generated automatically from AI good faith analysis
		...
		try {
			...
			try {
				...
				if (data.tags && Array.isArray(data.tags)) {
					aiGeneratedTags = validateTags(data.tags);
				}
										...
										<div class="citation-item">
											<div class="citation-content">
												...
												<div class="citation-details">
													...
													{#if citation.publish_date}({citation.publish_date}){/if}
													{#if citation.publisher}. {citation.publisher}{/if}
													{#if citation.page_number}, p. {citation.page_number}{/if}
													{#if citation.accessed_date}. Accessed: {citation.accessed_date}{/if}
												</div>
						...
						<div class="form-group">
							<div class="ai-tags-display">
								<div class="ai-tags-header">
									<span class="ai-label">🤖 AI-Generated Topics</span>
								</div>
								<div class="current-tags">
									{#each aiGeneratedTags as tag}
										<span class="tag ai-tag">{tag}</span>
									{/each}
								</div>
								<p class="ai-tags-note">
									Tags are automatically generated from your content during analysis
								</p>
							</div>
						</div>
					{/if}
...
<!-- Tags are now automatically generated by AI from content analysis -->
...
<style>
	...
	.ai-tags-display {
		...
		background: linear-gradient(
			135deg,
			color-mix(in srgb, var(--color-accent) 5%, var(--color-surface-alt)),
