...
<style>
  .profile-public-container { max-width: 900px; margin: 2rem auto; padding: 1rem; }
  .profile-header h1 { margin: 0 0 0.25rem 0; }
  .bio { color: var(--color-text-secondary); margin: 0.5rem 0 0.75rem; }
  .links { display:flex; gap:0.5rem; flex-wrap: wrap; }
  .links .link { color: var(--color-primary); text-decoration: none; display:inline-flex; align-items:center; gap:0.35rem; }
  .links .link.icon { width: 28px; height: 28px; border-radius: 50%; display:inline-flex; align-items:center; justify-content:center; background: color-mix(in srgb, var(--color-primary) 10%, transparent); color: var(--color-primary); }
  .links .link.icon:hover { background: color-mix(in srgb, var(--color-primary) 18%, transparent); }
  .links .link:hover { text-decoration: none; }
  .sr-only { position: absolute; width: 1px; height: 1px; padding: 0; margin: -1px; overflow: hidden; clip: rect(0, 0, 0, 0); white-space: nowrap; border: 0; }
  .profile-section { margin-top: 1.5rem; }
  .list { list-style: none; margin: 0.5rem 0 0; padding: 0; display: flex; flex-direction: column; gap: 0.4rem; }
  .item a { color: var(--color-text-primary); text-decoration: none; }
