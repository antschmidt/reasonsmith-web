<script lang="ts">
	...
	const handleWebAuthnSignIn = async () => {
		...
		try {
			...
			const challengeResponse = await fetch(`${nhostAuthUrl}/v1/signin/webauthn`, {
				method: 'POST',
				headers: {
			...
			const verifyResponse = await fetch(`${nhostAuthUrl}/v1/signin/webauthn/verify`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
	...
	<main class="auth-main">
		<div class="auth-panel two-column">
			...
			<section class="auth-intro">
				<h2>Learn through constructive dialogue.</h2>
				<p>
					ReasonSmith is a platform for developing critical thinking skills through evidence-based
					discussion. Practice steelmanning, improve your reasoning, and build research literacy
					through collaborative refinement.
				</p>
				<p class="pull-quote">
					"The best way to strengthen your thinking is to genuinely understand perspectives you
					disagree with."
				</p>
				<p>
					Sign in to explore discussions, practice constructive dialogue, and develop your skills as
