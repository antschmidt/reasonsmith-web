<script lang="ts">
	...
	const fallacies: Record<string, Info> = {
		'Straw man': {
			...
			detail:
				'A weaker distorted version is rebutted instead of the actual claim, generating irrelevant refutations.',
			examples: [
		...
		'Motte-and-Bailey': {
			...
			detail:
				'Advance sweeping “bailey,” retreat to truistic “motte” under scrutiny while implying equivalence.',
			examples: [
	...
	const cultish: Record<string, Info> = {
		'Thought-Terminating Cliché': {
			...
			examples: ['“It is what it is.”', "“That's just fear talking.”", '“Stop overthinking it.”']
		},
		'Purity Spiral': {
	...
	function openItem(kind: 'fallacy' | 'cultish', name: string) {
		activeKind = kind;
		activeItem = name;
	}
	function closeModal() {
	...
	function onKey(e: KeyboardEvent, kind: 'fallacy' | 'cultish', name: string) {
...
<div class="guide-container">
	...
	<section>
		...
		<ul class="bullet">
			<li>
				<strong>Good Faith First</strong>: Engage to understand, refine, or test ideas—not to score
				rhetorical wins.
			</li>
			<li>
				<strong>Evidence over Assertion</strong>: Claims that influence conclusions should cite or
				label speculation clearly.
			</li>
			<li>
				<strong>Precision over Volume</strong>: Better one well-structured paragraph than five
				meandering ones.
			</li>
			<li>
				<strong>Steelman Before Critique</strong>: Accurately restate what you’re responding to (or
				ask clarifying questions).
			</li>
			<li>
				<strong>Separate Person from Position</strong>: Challenge reasoning, data, assumptions—not
				identity.
			</li>
		</ul>
	</section>
	...
	<section>
		...
		<ul class="metric-list">
			<li>
				<strong>Good-Faith Structure Score</strong>: Presence of steelmanning, explicit assumptions,
				delineated claims vs. support.
			</li>
			<li>
				<strong>Fallacy Burden</strong>: Frequency + severity of detected logical fallacy patterns
				(weighted: straw man > slippery slope > ad hominem aside, etc.).
			</li>
			<li>
				<strong>Cultish Language Index</strong>: Density of manipulative rhetorical markers
				(thought-terminating clichés, purity framing, loaded binaries).
			</li>
			<li>
				<strong>Source Integrity</strong>: Ratio of claims with adequate citations + citation tier
				quality.
			</li>
			<li>
				<strong>Responsiveness</strong>: Whether replies address strongest prior point rather than
	...
	<section>
		...
		<ul class="bullet">
			<li>
				<strong>Tracked Fallacies (Examples)</strong>:
			...
			<li>
				<strong>Cultish / Manipulative Signals</strong>:
				<span class="fallacy-tags">
			...
			<li>
				<strong>Transparency</strong>: You will be able to see what was flagged and why. Dispute
...
If you still deny this, you're willfully discarding multi-domain convergent evidence."</code
					></pre>
			</div>
			<div>
	...
	<section>
		...
		<ul class="bullet">
			...
			<li>
				<strong>Appeal Workflow (Planned)</strong>: See extracted features, challenge
				misclassification, supply clarifying context.
			</li>
			<li>
				<strong>Model Feedback Loop</strong>: Successful appeals feed back as counterexamples.
			</li>
		</ul>
	</section>
						...
						{#if currentItem}
							...
							{#if currentItem.examples}
								<div class="example-block">
									...
									<ul class="examples">
										{#each currentItem.examples as ex}<li>{ex}</li>{/each}
									</ul>
								</div>
							{/if}
						{/if}
...
<style>
	...
	.fallacy-chip.cultish {
		border-color: var(--color-accent);
	}
	.fallacy-chip.cultish:hover,
	.fallacy-chip.cultish:focus {
