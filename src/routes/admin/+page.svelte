<script lang="ts">
  ...
  import { UPDATE_CONTRIBUTOR_ANALYSIS_SETTINGS, RESET_ANALYSIS_USAGE, UPDATE_CONTRIBUTOR_ROLE } from '$lib/graphql/queries';
  ...
  async function updateAnalysisSettings(contributorId: string, analysisEnabled: boolean, analysisLimit: number | null) {
    saving = true;
    error = null;
    success = null;
          ...
          <tbody>
            {#each contributors as contributor (contributor.id)}
              <tr class="contributor-row">
                <td class="user-info">
                  <div class="user-name">{contributor.display_name || contributor.handle || 'Unnamed'}</div>
                ...
                <td>
                  <label class="toggle">
                    <input
                      ...
                      on:change={(e) => updateAnalysisSettings(contributor.id, e.currentTarget.checked, contributor.analysis_limit)}
                    />
