...
export function calculateUserStats(data: RawUserStatsData): UserStats {
  ...
  const allContent = [
    ...posts.map(p => ({ good_faith_score: p.good_faith_score, good_faith_label: p.good_faith_label })),
    ...discussions.map(d => ({ good_faith_score: d.good_faith_score, good_faith_label: d.good_faith_label }))
  ];
  
  // Calculate Good Faith Rate
  const contentWithScores = allContent.filter(item => 
    item.good_faith_score !== null && item.good_faith_score !== undefined
  );
  
  let goodFaithRate = 0;
  if (contentWithScores.length > 0) {
    const goodFaithThreshold = 0.6; // Consider anything above 60% as good faith
    const goodFaithCount = contentWithScores.filter(item => 
      (item.good_faith_score || 0) >= goodFaithThreshold
    ).length;
  ...
  const postsWithCitations = posts.filter(post => 
    post.style_metadata && 
