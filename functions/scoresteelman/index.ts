...
// Serverless function to score a post for steelman quality using Claude (Anthropic)
// Expected request body: { postId: string, content: string, discussionContext?: string }
...
export default async function handler(req: Request): Promise<Response> {
	...
	try {
		...
		return new Response(JSON.stringify({ ...scored, postId: postId || null }), {
			status: 200,
			headers: { 'Content-Type': 'application/json' }
		});
	} catch (e: any) {
