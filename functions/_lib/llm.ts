...
function mapLabel(score: number): string {
	if (score >= 0.8) return 'constructive';
	...
	try {
			...
			messages: [
				{
					role: 'system',
					content:
						'You are an impartial assistant that evaluates whether a discussion message is written in good faith. Return ONLY valid JSON with fields: score (0..1), label (string), rationale (short constructive feedback), flags (array of short machine-friendly slugs identifying issues like ad_hominem, sarcasm, hostility, unsupported_claim). Keep rationale <= 240 chars. If text is empty set score=0 and label="hostile".'
				},
		...
		const label =
			typeof parsed.label === 'string' && parsed.label.trim()
				? parsed.label.trim().toLowerCase()
				: mapLabel(scoreNum);
		const rationale =
			typeof parsed.rationale === 'string' ? parsed.rationale.trim().slice(0, 500) : undefined;
		const flags = Array.isArray(parsed.flags)
			? parsed.flags.filter((f: any) => typeof f === 'string').slice(0, 12)
			: undefined;
